...
class TestBase:
    ...
    def test_get_spaces(self, app_client_user):
        response = app_client_user.get(path=f'/get_spaces?scope=test')
        assert response.status_code == 200

    def test_space_content(self, app_client_user):
        response = app_client_user.get(path=f'/get_space/abd9cd7f-9ffd-42b0-bce4-eb14b51a1fd1?scope=test')
        assert response.status_code == 200

        response = app_client_user.get(path=f'/get_space/abd9cd7f-9ffd-42b0-bce4-eb14b51a1fd0?scope=test')
