...
class TestViewFileById:
    ...
    def test_file_damaged(self, api_log_client):
        response = api_log_client._request('GET', url + f'/file/abd9cd7f-9ffd-41b0-bce4-eb14b51a6d72/view')
        assert response.status_code == 404
        assert response.text == '{"error":"File not found"}\n'

    def test_file_success(self, api_log_client):
        response = api_log_client._request('GET', url + f'/file/abd9cd7f-9ffd-41b0-d1e4-eb14b51a6d72/view')
