...
import { useCreateDirLazy } from 'api';
...
function SpacePage(props: SpacePageProps) {
    ...
    let create = useCreateDirLazy();
    const handleFabClick = React.useCallback((event: React.MouseEvent<HTMLButtonElement>) => {
        },
        []
    );
    ...
    return (
        <Stack height='100%' divider={<Divider />}>
            ...
            <Box height='100%'>
                ...
                <Fab size='small' color='secondary' aria-label='add' onClick={handleFabClick}>
                    <AddIcon/>
                </Fab>
            </Box>
        </Stack>
